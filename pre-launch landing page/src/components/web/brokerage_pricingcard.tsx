...
export default function BrokeragePricingSection() {
  ...
  const t = useTranslations("brokeragePricing");

  const plans = [
    {
      name: "Brokerage Lite",
      freeLabel: "Free",
      description: "Essential visibility for new brokerages.",
      earlyRate: null,
      standardRate: null,
      monthlyCredits: "00",
    ...
    },
    {
      name: "Brokerage Pro",
      freeLabel: null,
      description: "Advanced insights for established offices.",
      earlyRate: "AED 1,499",
    ...
    },
    {
      name: "Enterprise",
      freeLabel: null,
      description: "Maximum scale for multi-office ",
      earlyRate: "AED 2,999",
...
<div className="relative flex items-center gap-3 sm:gap-4 px-4 sm:px-6 md:px-8 min-h-[80px] sm:min-h-[100px] lg:min-h-[120px] 2xl:min-h-[135px] 
  bg-gradient-to-r from-[#7FFFD4] to-[#E5FFF6] dark:bg-none dark:from-black dark:to-black">
          {/* Icon */}
      <svg  className="w-[33.349px] h-[33.451px] sm:w-[41px] sm:h-[41px]" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 41 41" fill="none">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 p-2 mt-6">
          {plans.map((plan) => (
            <div key={plan.name} className="relative flex flex-col">
              {/* Most Popular Badge */}
              {plan.isMostPopular && (
                <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2 px-3 py-1 bg-[#7FFFD4] rounded-[0_12px_0_12px] text-xs font-medium" style={{ top: "-10px" }}>
              ...
              <div
                className={`flex flex-col h-full rounded-2xl overflow-hidden ${
                  plan.isMostPopular ? "border border-[#7FFFD4] bg-[#7FFFD4]/20" : "bg-white dark:bg-[#111] dark:border dark:border-[#333]"
                }`}
                style={{
                  boxShadow: plan.isMostPopular ? "none" : "0 0 10px rgba(0,0,0,0.10)",
                  paddingTop: "56px",
                }}
              >
